package com.example.commonlib.date;
    ...
    */
    ...
    private final class CustomLocalization {
        CustomLocalization(List<String> aMonths, List<String> aWeekdays, List<String> aAmPm) {
            if (aMonths.size() != 12) {
                throw new IllegalArgumentException("Your List of custom months must have size 12, but its size is "
                        + aMonths.size());
            }
            if (aWeekdays.size() != 7) {
                throw new IllegalArgumentException("Your List of custom weekdays must have size 7, but its size is "
                        + aWeekdays.size());
            }
            if (aAmPm.size() != 2) {
            ...
            mMonths = aMonths;
            mWeekdays = aWeekdays;
            mAmPmIndicators = aAmPm;
        }

        List<String> mMonths;
        List<String> mWeekdays;
        List<String> mAmPmIndicators;
    }
    ...
    private static final class InterpretedRange {
        ...
        public String toString() {
            return "Start:" + mStart + " End:" + mEnd + " '" + mText + "'";
        };
    }
    ...
    */
    ...
    private void findEscapedRanges() {
        ...
        while (matcher.find()) {
            ...
            escapedRange.mStart = matcher.start(); //first pipe
            escapedRange.mEnd = matcher.end() - 1; //second pipe
    ...
    private boolean isInEscapedRange(InterpretedRange aInterpretedRange) {
        ...
        for (EscapedRange escapedRange : fEscapedRanges) {
            ...
            if (escapedRange.mStart <= aInterpretedRange.mStart && aInterpretedRange.mStart <= escapedRange.mEnd) {
                result = true;
                break;
            }
        }
        ...
        for (String token : TOKENS) {
            ...
            while (matcher.find()) {
                ...
                if (!isInEscapedRange(interpretedRange)) {
                    interpretedRange.mText = interpretThe(matcher.group(), aDateTime);
    ...
    */
    ...
    private String produceFinalOutput() {
        ...
        while (idx < fFormat.length()) {
            ...
            if (interpretation != null) {
                result.append(interpretation.mText);
                idx = interpretation.mEnd;
            } else {
    ...
    private InterpretedRange getInterpretation(int aIdx) {
        ...
        for (InterpretedRange interpretedRange : fInterpretedRanges) {
            if (interpretedRange.mStart == aIdx) {
    ...
    private String lookupCustomMonthFor(Integer aMonth) {
        ...
        if (fCustomLocalization == null || fCustomLocalization.mMonths == null) {
            return EMPTY_STRING;
        }
        return fCustomLocalization.mMonths.get(aMonth - 1);
    }
    ...
    private String lookupCustomWeekdayFor(Integer aWeekday) {
        if (fCustomLocalization == null || fCustomLocalization.mWeekdays == null) {
            return EMPTY_STRING;
        }
        return fCustomLocalization.mWeekdays.get(aWeekday - 1);
    }
    ...
    private String lookupCustomAmPmFor(Integer aHour) {
        String result = EMPTY_STRING;
        if (fCustomLocalization != null && fCustomLocalization.mAmPmIndicators != null) {
            if (aHour < 12) {
                result = fCustomLocalization.mAmPmIndicators.get(AM);
            } else {
                result = fCustomLocalization.mAmPmIndicators.get(PM);
            }
        }
        return result;
    }
